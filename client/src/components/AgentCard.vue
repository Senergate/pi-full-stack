...
const nextBatteryCommand = computed(() => displayedState.value.batteryCharging !== true);

const mergedPendingState = computed(() => {
  const base = prediction.value?.state ?? normalizedDeviceStates.value;
  return {
    heatpump: pendingDevices.heatpump ? pendingTargetState.heatpump : base.heatpump,
    wallbox: pendingDevices.wallbox ? pendingTargetState.wallbox : base.wallbox,
    batteryCharging: pendingDevices.batteryCharging ? pendingTargetState.batteryCharging : base.batteryCharging,
  };
});
