...
assert(snap.projectedCurrentsA.b === 67, 'Wallbox R0 must represent two 16 A wallboxes and add 32 A to virtual L2.');
...
assert(snap.wallbox.r0 === false && snap.wallbox.r1 === false, 'AI test scenario must start with both wallbox relays OFF.');
...
snap = SimulationRuntime.getSnapshot();
assert(snap.heatpumpLevel === 0, 'Reset must stop the simulated heatpump.');
