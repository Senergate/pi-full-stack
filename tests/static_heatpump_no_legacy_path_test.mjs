...
const deny = [
  'targetHeatpump / HEATPUMP_LEVELS',
  'safeLoad * 10',
  'EspService.heatpump(0.8',
  'EspService.heatpump(0.6',
  'EspService.heatpump(0.4',
];
...
for (const file of scanRoots.flatMap(walk)) {
  ...
  for (const term of deny) {
    if (text.includes(term)) throw new Error(`${term} still exists in ${path.relative(root, file)}`);
  }
}
